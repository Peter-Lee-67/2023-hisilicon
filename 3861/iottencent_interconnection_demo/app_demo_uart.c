...
/* publish sample */
hi_void IotPublishSample(char Buffer)
{
    /* reported attribute */
    WeChatProfile weChatProfile = {
        ...
        .reportAction.temperatureData = Buffer-'a', /* 30 :temperature data */
        //.reportAction.buffer = Buffer,
        /* report humidity */
...
static hi_void *DemoEntry(char *param)
{
    ...
    while (1) {
        ...
        IotPublishSample(uartDefConfig.g_receiveUartBuff[5]); // 发布例程
#endif
...
hi_void UartTransmit(hi_void)
{
    ...
    attr.stack_size = UART_DEMO_TASK_STAK_SIZE+CN_IOT_TASK_STACKSIZE;
