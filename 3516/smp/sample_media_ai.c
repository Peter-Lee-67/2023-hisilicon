...
static HI_VOID HandClassifyAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    ...
    if (GetCfgBool("hand_classify_switch:support_hand_classify", true)) {
        if (g_workPlug.model == 0) {
            ...
            // Osd Initialization
            ret = OsdLibInit();
            g_osdsTrash = g_aicMediaInfo.osds;
        ...
        end_lee = clock();
        // 更新帧率
        double time_lee = (double)(end_lee - start_lee) / CLOCKS_PER_SEC;
        double fps = 1.0 / time_lee;
        // 写入字符数组
        HI_S32 offset = 0;
        HI_S32 size = sizeof(osdBuf);
        offset += snprintf_s(osdBuf + offset, size - offset, size - offset - 1, "Defect detection: {");
        // sprintf(osdBuf, "fps:%.2f", fps);
        offset += snprintf_s(osdBuf + offset, size - offset, size - offset - 1,
            "fps: %u", (int)fps);
        offset += snprintf_s(osdBuf + offset, size - offset, size - offset - 1, " }");
    ...
    while (HI_FALSE == g_bAiProcessStopSignal) {
       ...
       VIDEO_FRAME_S
        ...
        typedef struct hiVIDEO_FRAME_S{
            ...
            HI_U64 u64HeaderVirAddr[3];         // 压缩头虚拟地址
            HI_U64 u64PhyAddr[3];               // 图像数据物理地址
            HI_U64 u64VirAddr[3];               // 图像数据虚拟地址
            HI_U64 u64ExtPhyAddr[3];            // 10bit 数据位宽的图像，有些数据格式的存储方式是前8bit 和后 2bit 分开存储，这里指后 2bit 数据的物理地址。
            HI_U64 u64ExtVirAddr[3];            // 10bit 数据位宽的图像，有些数据格式的存储方式是前8bit 和后 2bit 分开存储，这里指后 2bit 数据的虚拟地址。
...
static HI_S32 HandClassifyAiThreadProcess(HI_VOID)
{
    ...
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    // 重点关注：GetVpssChnFrameHandClassify
