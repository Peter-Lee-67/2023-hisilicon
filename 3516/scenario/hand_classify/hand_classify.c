...
*/
#define HAND_FRM_WIDTH     512
#define HAND_FRM_HEIGHT    512
...
#define AUDIO_FRAME        10       // Recognize once every 15 frames, can be configured by yourself
    ...
    if (GetCfgBool("audio_player:support_audio", true)) {
        ...
        if(AUDIO_THREAD_CREATE == 1){
            ...
            if (ret != 0) {
                ...
                SAMPLE_PRT("audio proccess thread creat success!\n");
            }
        }
    }
 ...
 * Hand gesture recognition info
 */
static void DefectDetectFlag(const RecogNumInfo resBuf)
{
    HI_CHAR *gestureName = NULL;
    switch (resBuf.num) {
        case 0u:
            gestureName = "Longitudinal Crack ";
            UartSendRead(uartFd, FistGesture); // 拳头手势
            SAMPLE_PRT("----defect type----:%s\n", gestureName);
            break;
        case 1u:
            gestureName = "Transverse Crack";
            UartSendRead(uartFd, ForefingerGesture); // 食指手势
            SAMPLE_PRT("----defect type----:%s\n", gestureName);
            break;
        case 2u:
            gestureName = "Alligator Crack";
            UartSendRead(uartFd, OkGesture); // OK手势
            SAMPLE_PRT("----defect type----:%s\n", gestureName);
            break;
        case 3u:
            ...
            break;
        default:
            gestureName = "defect others";
            UartSendRead(uartFd, InvalidGesture); // 无效值
            SAMPLE_PRT("----defect type----:%s\n", gestureName);
            break;
    }
    SAMPLE_PRT("Defect detect success\n");
}


static HI_S32 count_history[4] = {0, 0, 0, 0};
/*
 * 手部检测和手势分类推理
...
HI_S32 Yolo2HandDetectResnetClassifyCal(uintptr_t model, VIDEO_FRAME_INFO_S *srcFrm, VIDEO_FRAME_INFO_S *dstFrm)
{
    ...
    HI_FLOAT maxScore = -1;
    HI_S32 maxScoreClass = 0;
    HI_S32 count[4] = {0, 0, 0, 0};
    ...
    for (int i = 0; i < objNum; i++) {
        ...
        boxs[i] = *box;
        // lzy
        if(maxScore < objs[i].score){
            maxScore = objs[i].score;
            maxScoreClass = objs[i].cls;
        }
        count[objs[i].cls] += 1;
    }
    ...
    if (biggestBoxIndex >= 0) {
        ...
        // int MppFrmDrawRects(VIDEO_FRAME_INFO_S *frm, const RectBox *boxes, int boxesNum, uint32_t color, int thick);
    ...
    */
    // lzy
    resBuf[0].num = (HI_U32)maxScoreClass;
    resBuf[0].score = (HI_U32)(maxScore * 1000);
    // SAMPLE_PRT("resBuf[0].num=%d, resBuf[0].score=%d", resBuf[0].num, resBuf[0].score);

    DefectDetectFlag(resBuf[0]);

    SAMPLE_PRT("objNum:%d\n", objNum);
    SAMPLE_PRT("class0:%d, class1:%d, class2:%d, class3:%d (cur_count)\n", count[0], count[1], count[2], count[3]);
    SAMPLE_PRT("class0:%d, class1:%d, class2:%d, class3:%d (his_count)\n", count_history[0], count_history[1], count_history[2], count_history[3]);

    // lzy
    bool isChanged = false;
    HI_S32 broadcastClass = -1;
    for(int i = 0; i < 4; i++){
        if(count[i] != count_history[i]){
            isChanged = true;
            count_history[i] = count[i];
        }
        if(count[i] > 0){
            broadcastClass = i;
        }
    }
    resBuf[1].num = (HI_U32)broadcastClass;
    resBuf[1].score = (HI_U32)(1);
    
    SAMPLE_PRT("resBuf[1].num=%d", resBuf[1].num);

    SAMPLE_PRT("isChanged:%d\n", isChanged);

    if (GetCfgBool("audio_player:support_audio", true)) {
        if (broadcastClass >= 0 && isChanged){
            SAMPLE_PRT("Enter FdWriteMsg\n");
            if(AUDIO_THREAD_CREATE == 1){
                // if (FdWriteMsg(g_stmChn.out, &resBuf[0], sizeof(RecogNumInfo)) != sizeof(RecogNumInfo)) {
                //     SAMPLE_PRT("FdWriteMsg FAIL\n");
                // }
                if (FdWriteMsg(g_stmChn.out, &resBuf[1], sizeof(RecogNumInfo)) != sizeof(RecogNumInfo)) {
                    SAMPLE_PRT("FdWriteMsg FAIL\n");
                }
            }
            else{
                AudioTest(resBuf[0].num, -1);
            }
        }
    }
    return ret;
